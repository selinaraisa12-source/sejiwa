...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($password !== $confirm) {
    ...
    $stmt = $conn->prepare("INSERT INTO tb_users (nama, email, password, panjang_password, role) VALUES (?, ?, ?, ?, 0)");
    $stmt->bind_param("sssi", $nama, $email, $password_hash, $panjang_password);
    ...
    if ($stmt->execute()) {
      ...
      $error = "Terjadi kesalahan saat menyimpan data. Email mungkin sudah terdaftar.";
    }
  }
}
?>
...
/* ===== Wrapper & brand ===== */
.wrap{width:min(980px,96vw); display:flex; flex-direction:column; align-items:center; margin-top:24px; gap:18px}
...
.panel-inner{ padding:32px 28px 30px }
.form-title{
...
@media (prefers-reduced-motion: reduce){
  .page{transition:none}.logo,.glow{animation:none}
}
</style>
</head>
<body>
  ...
  <div class="wrap">
    ...
    <section class="panel">
      <div class="panel-inner">
        ...
        <p class="form-sub">Isi data lengkap untuk mulai perjalananmu bersama Sejiwa.</p>

        <?php if (!empty($error)) echo "<div class='error'><i class='fa-solid fa-triangle-exclamation'></i> $error</div>"; ?>

        <form method="post" class="form" novalidate>
          <div class="group"><input class="input" type="text" name="nama" placeholder="Nama Lengkap" required></div>
          <div class="group"><input class="input" type="email" name="email" placeholder="Alamat Email" required></div>
          <div class="group">
            <input class="input" type="password" name="password" placeholder="Kata Sandi" id="regPassword" required>
            <span class="eye" onclick="togglePassword('regPassword')"><i class="fa-solid fa-eye" id="eyeIcon"></i></span>
          </div>
          <div class="group"><input class="input" type="password" name="confirm" placeholder="Konfirmasi Kata Sandi" required></div>
          <button type="submit" class="btn">DAFTAR</button>
          <div class="switch">Sudah punya akun? <a href="awal.php">Masuk</a></div>
        </form>
      </div>
    </section>
  </div>
...
document.addEventListener('DOMContentLoaded', ()=>{
  document.querySelector('.page').classList.add('enter');
});
</script>
</body>
</html>
